...
## Steps to Create, Build, Load and Debug using the FSBL_XIP Template csolution project
...
- Select `target` and `template` **FSBL_XIP** (from DFP) and click **Create**
...
> STM32CubeMX configuration example for the [STM32N6570-DK board](https://github.com/Open-CMSIS-Pack/STM32N6570-DK_BSP/blob/main/Examples/FSBL_XIP/Configuration.md#configuration-in-stm32cubemx)
  ...
  ```yaml
    ...
    executes:
      ...
      - execute: Copy_FSBL_hex_to_root
        ...
        input:
          - ../out/FSBL/<target>/<build-type>/FSBL.hex
        output:
          - ../FSBL.hex
      - execute: Copy_FSBL_axf_to_root
        run: ${CMAKE_COMMAND} -E copy $input$ $output$
        input:
          - ../out/FSBL/<target>/<build-type>/FSBL.axf
        output:
          - ../FSBL.axf
  ```
...
- Add linker script
  ...
  ```yaml
    # Linker script definition
    linker:
      - script: ../STM32CubeMX/<target>/STM32CubeMX/MDK-ARM/Appli/<target>_ROMxspi2.sct
        for-compiler: AC6
  ```
...
> STM32CubeMX configuration example for the [STM32N6570-DK board](https://github.com/Open-CMSIS-Pack/STM32N6570-DK_BSP/blob/main/Examples/FSBL_XIP/Configuration.md#configuration-in-vscode)
    ...
    - **FLASH MODE**
        ...
        - Open `.vscode\launch.json` file and modify configuration named "STLink@pyOCD (launch)"
            ...
            "initCommands": [
                ...
                "continue",
                "symbol-file \"out/Appli/<target>/<build-type>/Appli.axf\"",
                "tbreak main"
            ```
